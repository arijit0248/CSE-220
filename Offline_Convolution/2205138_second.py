...
def readFromFile(fileName):
    ...
    values = np.array([float(v) for v in secondLine])
    INF = max(abs(start), abs(end))
    signal = Signal(INF)

    for i in range(len(values)):
        t = start + i
        signal.set_value_at_time(t, values[i])
    return signal


if __name__ == "__main__":
    inputSignal = readFromFile("input_signal.txt")
    inputSignal.plot("Noisy Signal")
    h = Signal(2)
    h.set_value_at_time(-2, 1.0/5.0)
    h.set_value_at_time(-1, 1.0/5.0)
    h.set_value_at_time(0, 1.0/5.0)
    h.set_value_at_time(1, 1.0/5.0)
    h.set_value_at_time(2, 1.0/5.0)
    ltiSystem = LTI_System(h)
    outputSignal = ltiSystem.output(inputSignal)
    outputSignal.plot("Smoothed Signal")
