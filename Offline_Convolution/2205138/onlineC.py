...
def multiply_polynomials():
    ...
    # Calculate required INF
    # The result spans indices 0 to d1+d2
    # We need enough buffer to avoid any boundary issues
    result_degree = d1 + d2
    INF = result_degree + 100  # Large enough buffer

    # Create input signal (first polynomial coefficients)
    # Coefficients go at time indices 0, 1, 2, ..., d1
    input_signal = Signal(INF)
    for i in range(len(coeffs1)):
        input_signal.set_value_at_time(i, coeffs1[i])

    # Create impulse response (second polynomial coefficients)
    # Coefficients go at time indices 0, 1, 2, ..., d2
    impulse_response = Signal(INF)
    for i in range(len(coeffs2)):
        impulse_response.set_value_at_time(i, coeffs2[i])

    # Perform convolution using LTI system
    system = LTI_System(impulse_response)
    output_signal = system.output(input_signal)

    # Extract result coefficients from time indices 0 to d1+d2
    result_coeffs = []
    for i in range(result_degree + 1):
        coeff = output_signal.get_value_at_time(i)
        result_coeffs.append(int(round(coeff)))

    # Print output
    print(f"Degree of the Polynomial: {result_degree}")
    print(f"Coefficients: {' '.join(map(str, result_coeffs))}")
