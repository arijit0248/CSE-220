...
impulse2 = Signal(n)
for i in range(n):
    impulse1.set_value_at_time(i, (1/n))
temp = (n*(n+1))/2
for i in range(n):
    impulse2.set_value_at_time(i, (n-i)/temp)
lti1 = LTI_System(impulse1)
lti2 = LTI_System(impulse2)

output1 = lti1.output(inputSignal)
output2 = lti2.output(inputSignal)
# Please determine uma and wma.

# Unweighted Moving Averages as a Python list
uma = []
for i in range(n-1, len(price_list)):
    uma.append(output1.get_value_at_time(i))

# Weighted Moving Averages as a Python list
wma = []
for i in range(n-1, len(price_list)):
    wma.append(output2.get_value_at_time(i))
